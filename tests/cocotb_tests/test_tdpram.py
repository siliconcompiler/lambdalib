...
if _has_cocotb:
    ...
    async def test_tdpram_simultaneous_ops(dut):
        ...
        assert read_a == test_val_a, \
            f"Port A read mismatch: got 0x{read_a:X}, expected 0x{test_val_a:X}"
        ...
        assert read_a == test_val_a, \
            f"Port A final read mismatch: got 0x{read_a:X}, expected 0x{test_val_a:X}"
        assert read_b == test_val_b, \
            f"Port B final read mismatch: got 0x{read_b:X}, expected 0x{test_val_b:X}"
    ...
    async def test_tdpram_port_isolation(dut):
        ...
        read_a = int(dut.dout_a.value)
        assert read_a == test_val_a, \
            f"Port A isolation failed: got 0x{read_a:X}, expected 0x{test_val_a:X}"
